...
fn main() {
    ...
    println!("- Lattice size: {}×{}×{} ({} spins)", lattice_size, lattice_size, lattice_size, 
...
fn demonstrate_3d_phase_transition(lattice_size: usize) {
    ...
    for &target_step in &time_points {
        ...
        println!("Step {:4}: E = {:8.4}, |M| = {:6.4} (smoothed: {:6.4})", 
                ising.step,
    ...
    println!("- Energy per site at T=0 (2D): {:.1}", analysis::energy_per_site_at_zero_temp_2d());
    println!("- Energy per site at T=0 (3D): {:.1}", analysis::energy_per_site_at_zero_temp_3d());
