use md::ising::{analysis, IsingModel4D};
...
fn main() {
    ...
    println!(
        "- Mean field prediction: T_c ≈ {:.1}",
        analysis::mean_field_critical_temperature(4)
    );
...
fn analyze_upper_critical_dimension(results: &[SimulationResult], t_critical: f64) {
    ...
    println!(
        "- Deviation from MF: {:.1}%",
        100.0 * (t_c_mf - t_c_4d).abs() / t_c_4d
    );
    println!();
    ...
    println!(
        "- 4D: E₀ = {:.1} J per site",
        analysis::energy_per_site_at_zero_temp_4d()
    );
    println!();

    // Find the critical region results
    let critical_region: Vec<_> = results
        .iter()
...
fn performance_comparison() {
    ...
    for &size in &sizes {
        ...
        println!(
            "Size {}⁴ ({:6} spins): {:8.2} ms ({:6.0} flips/ms)",
            size,
            total_spins,
            duration.as_millis(),
