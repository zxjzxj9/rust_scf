...
fn compare_algorithms_at_temperature(lattice_size: usize, temperature: f64, steps: usize) {
    ...
    let metro_mean_energy =
        energy_samples_metro.iter().sum::<f64>() / energy_samples_metro.len() as f64;
    ...
    let wolff_mean_energy =
        energy_samples_wolff.iter().sum::<f64>() / energy_samples_wolff.len() as f64;
    ...
    println!(
        "  Max Cluster Size: {:6} spins ({:.1}% of lattice)",
        max_cluster_size,
    ...
    println!(
        "  Magnetization Difference: {:6.6} (should be small)",
        mag_diff
    );
}
...
fn analyze_cluster_sizes(lattice_size: usize, t_critical: f64) {
    ...
    for (i, &temp) in temperatures.iter().enumerate() {
        ...
        println!(
            "  Mean cluster size: {:8.1} spins ({:.2}% of lattice)",
            mean_size,
            100.0 * mean_size / (lattice_size * lattice_size) as f64
        );
...
fn analyze_parallel_performance(lattice_size: usize, t_critical: f64) {
    ...
    println!(
        "  Time for 100 parallel clusters: {:6.1} ms",
        cluster_time.as_millis()
    );
    println!(
        "  Mean cluster size: {:8.1} spins ({:.2}% of lattice)",
        mean_cluster_size,
    ...
    println!(
        "  Max cluster size: {:8} spins ({:.1}% of lattice)",
        max_cluster_size,
        100.0 * max_cluster_size as f64 / (lattice_size * lattice_size) as f64
    );
    ...
    println!("\nKey Insights:");
    println!(
        "- Parallel ensemble sampling provides {}x speedup",
        speedup as u32
    );
    println!(
        "- Efficiency of {:.0}% indicates good thread utilization",
        efficiency
    );
