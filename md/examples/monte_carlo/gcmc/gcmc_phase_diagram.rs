...
use md::{parallel_gcmc_sweep, GCMC};
...
fn main() {
    ...
    for &temperature in temperatures.iter() {
        ...
        let mu_values: Vec<f64> = (0..n_mu).map(|i| mu_min + i as f64 * mu_step).collect();
...
fn analyze_phase_behavior(results: &[md::GCMCResults], temperature: f64) {
    ...
    println!(
        "  Low density (gas-like) points: {}",
        low_density_region.len()
    );
    println!(
        "  High density (liquid-like) points: {}",
        high_density_region.len()
    );

    // Find maximum density fluctuation (indicator of critical region)
    let max_fluct = results
        .iter()
        .map(|r| r.density_std())
        .fold(0.0_f64, |a, b| a.max(b));

    if let Some(critical_point) = results
        .iter()
        .find(|r| (r.density_std() - max_fluct).abs() < 1e-6)
    {
        println!(
            "  Maximum density fluctuation σ(ρ) = {:.4} at μ = {:.2}",
    ...
    for result in results.iter().filter(|r| r.avg_n_particles > 5.0) {
        ...
        if n_std > 0.0 {
            ...
            if compressibility > 1.0 {
                println!(
                    "    μ = {:.2}: χ_T ≈ {:.2} (enhanced near critical point)",
...
fn save_isotherm(results: &[md::GCMCResults], filename: &str) {
    ...
    writeln!(
        file,
        "# Columns: μ, ⟨N⟩, ⟨ρ⟩, σ(ρ), σ(N), ⟨E⟩, ⟨E/N⟩, Acc_disp, Acc_ins, Acc_del"
    )
    .unwrap();
    writeln!(file, "#").unwrap();
