...
fn benchmark_2d_parallel_performance() {
    ...
    for &size in &sizes {
        ...
        let start = Instant::now();
        let (parallel_energies, parallel_mags) =
            model.parallel_ensemble_sampling(num_runs, steps_per_run);
        ...
        let seq_mean_energy =
            sequential_energies.iter().sum::<f64>() / sequential_energies.len() as f64;
        let par_mean_energy =
            parallel_energies.iter().sum::<f64>() / parallel_energies.len() as f64;
        let energy_diff = (seq_mean_energy - par_mean_energy).abs();

        assert!(
            energy_diff < 0.1,
            "Energy statistics differ too much: {:.6}",
            energy_diff
        );
    }
...
fn benchmark_ensemble_sampling() {
    ...
    println!(
        "Testing different ensemble sizes ({}×{} lattice, {} steps/run):",
        size, size, steps_per_run
    );
    println!("┌─────────────┬────────────────┬────────────────┬──────────────┐");
...
fn benchmark_thread_scaling() {
    ...
    println!(
        "Testing thread scaling ({}×{} lattice, {} runs, {} steps/run):",
...
fn benchmark_memory_efficiency() {
    ...
    for &size in &sizes {
        ...
        println!(
            "│ {:6}×{} │ {:9} B │ {:10}   │ {:13} KB │",
            size, size, model_size_bytes, num_threads, total_memory_kb
        );
    }
