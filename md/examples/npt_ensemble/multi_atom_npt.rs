...
use md::{LennardJones, NoseHooverParrinelloRahman, Integrator};
...
fn main() {
    ...
    println!("  Lattice: {}x{}x{} simple cubic", n_per_side, n_per_side, n_per_side);
    println!("  Lattice spacing: {:.2} σ", lattice_spacing);
    println!("  Initial box: {:.2} × {:.2} × {:.2}", 
             initial_box_length, initial_box_length, initial_box_length);
    println!("  Initial density: {:.3} atoms/σ³", 
    ...
    println!("  Steps 0-{}: {:.2} → {:.2} (gradual heating)", 
             temp_ramp_steps, initial_temp, final_temp);
    println!("  Steps {}-{}: {:.2} (equilibration)", 
             temp_ramp_steps, total_steps, final_temp);
    println!();

    // Output header  
    println!("┌────────┬─────────┬─────────┬─────────┬────────┬────────┬─────────┬──────────┐");
    println!("│ {:>6} │ {:>7} │ {:>7} │ {:>7} │ {:>6} │ {:>6} │ {:>7} │ {:>8} │", 
    ...
    for step in 0..total_steps {
        ...
        if step % output_interval == 0 {
            ...
            println!("│ {:>6} │ {:>7.3} │ {:>7.3} │ {:>7.3} │ {:>6.2} │ {:>6.2} │ {:>7.3} │ {:>8.5} │", 
                     step, 
                     current_temp,
                     current_pressure, 
                     target_pressure,
