...
fn main() {
    ...
    println!("  Atom 1: [{:.2}, {:.2}, {:.2}]", positions[0].x, positions[0].y, positions[0].z);
    println!("  Atom 2: [{:.2}, {:.2}, {:.2}]", positions[1].x, positions[1].y, positions[1].z);
    ...
    println!("\nEnergy: {:.6} ε", energy);
    println!("Forces:");
    println!("  Atom 1: [{:.4}, {:.4}, {:.4}]", forces[0].x, forces[0].y, forces[0].z);
    println!("  Atom 2: [{:.4}, {:.4}, {:.4}]", forces[1].x, forces[1].y, forces[1].z);
    println!("  Total: [{:.4}, {:.4}, {:.4}] (should be ~0)", 
             (forces[0] + forces[1]).x,
             (forces[0] + forces[1]).y,
             (forces[0] + forces[1]).z);
    ...
    println!("  Primitive cell volume: {:.2}", lattice_fcc.determinant().abs());
    ...
    println!("  Primitive cell volume: {:.2}", lattice_bcc.determinant().abs());
