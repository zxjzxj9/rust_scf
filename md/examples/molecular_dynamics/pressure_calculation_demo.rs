...
fn main() {
    ...
    println!("   Ideal gas P = N*k*T/V = {:.6}", positions.len() as f64 * temp / lj.volume());
    ...
    println!("   P (ideal) = {:.6}", 2.0 * positions.len() as f64 * temp / lj.volume());
    println!("   P (correction) = {:.6} (attractive)\n", pressure - 2.0 * positions.len() as f64 * temp / lj.volume());
    ...
    println!("   P (ideal) = {:.6}", 2.0 * positions.len() as f64 * temp / lj.volume());
    println!("   P (correction) = {:.6} (repulsive)\n", pressure - 2.0 * positions.len() as f64 * temp / lj.volume());
    ...
    for ix in 0..3 {
        for iy in 0..3 {
            for iz in 0..4 {
                ...
                positions.push(base);
                positions.push(base + Vector3::new(0.5 * lattice_constant, 0.5 * lattice_constant, 0.0));
                positions.push(base + Vector3::new(0.5 * lattice_constant, 0.0, 0.5 * lattice_constant));
                
                // Only add 4th atom for first 3 z-layers to get exactly 108
                if positions.len() < 108 {
                    positions.push(base + Vector3::new(0.0, 0.5 * lattice_constant, 0.5 * lattice_constant));
                }
            }
        }
    }
    ...
    let lj_liquid = LennardJones::new(epsilon, sigma, Vector3::new(box_size, box_size, 1.5 * box_size));
    ...
    let pressure_scalar = lj_aniso.compute_pressure_from_tensor(&positions_aniso, &velocities_aniso, mass);
    ...
    for i in 0..positions_aniso.len() {
        for alpha in 0..3 {
            for beta in 0..3 {
                kinetic_tensor[(alpha, beta)] += mass * velocities_aniso[i][alpha] * velocities_aniso[i][beta];
            }
        }
    }
    ...
    println!("   ┌                                      ┐");
    println!("   │ {:8.4}  {:8.4}  {:8.4} │",
             pressure_tensor[(0, 0)], pressure_tensor[(0, 1)], pressure_tensor[(0, 2)]);
    println!("   │ {:8.4}  {:8.4}  {:8.4} │",
             pressure_tensor[(1, 0)], pressure_tensor[(1, 1)], pressure_tensor[(1, 2)]);
    println!("   │ {:8.4}  {:8.4}  {:8.4} │",
             pressure_tensor[(2, 0)], pressure_tensor[(2, 1)], pressure_tensor[(2, 2)]);
    println!("   └                                      ┘");
    println!("\n   Diagonal elements:");
    println!("   P_xx = {:.6} (largest - particles/velocity along x)", pressure_tensor[(0, 0)]);
