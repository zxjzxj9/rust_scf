use md::ising::{IsingModel3D, analysis};
...
fn main() {
    ...
    println!("Setting up {}×{}×{} 3D Ising lattice at T = {:.2}", 
    ...
    println!("- Magnetization per site: {:.4}", ising.magnetization_per_site());
    println!("- Absolute magnetization: {:.4}", ising.abs_magnetization_per_site());
    ...
    println!("- 2D Critical Temperature: {:.4}", analysis::critical_temperature_2d());
    println!("- 3D Critical Temperature: {:.4}", analysis::critical_temperature_3d());
    println!("- Current Temperature: {:.4}", temperature);
    println!("- Current T/T_c(3D): {:.4}", temperature / analysis::critical_temperature_3d());
    ...
    for &temp in &temperatures {
        ...
        println!("T = {:.3}: E = {:6.3}, |M| = {:5.3}, Status: {}", 
                temp, 
                test_ising.energy_per_site(),
                test_ising.abs_magnetization_per_site(),
                if temp < analysis::critical_temperature_3d() { "Ordered" } else { "Disordered" });
    }
