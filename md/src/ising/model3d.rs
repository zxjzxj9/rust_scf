...
impl IsingModel3D {
    ...
    pub fn specific_heat(&self, energy_samples: &[f64]) -> f64 {
        ...
        variance
            / (self.temperature * self.temperature * (self.size * self.size * self.size) as f64)
    }
    ...
    pub fn print_slice(&self, z_level: usize) {
        if z_level >= self.size {
            println!(
                "Error: z_level {} exceeds lattice size {}",
                z_level, self.size
            );
            return;
        }
        ...
        println!(
            "Z-slice at level {} (size {}×{}):",
            z_level, self.size, self.size
        );
        for i in 0..self.size {
            for j in 0..self.size {
                print!(
                    "{:2}",
                    if self.spins[i][j][z_level] == 1 {
                        "↑"
                    } else {
                        "↓"
                    }
