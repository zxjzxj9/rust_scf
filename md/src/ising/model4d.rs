...
impl IsingModel4D {
    ...
    pub fn specific_heat(&self, energy_samples: &[f64]) -> f64 {
        ...
        let mean_energy_squared: f64 =
            energy_samples.iter().map(|e| e * e).sum::<f64>() / energy_samples.len() as f64;
    ...
    pub fn magnetic_susceptibility(&self, magnetization_samples: &[f64]) -> f64 {
        ...
        let mean_mag_squared: f64 = magnetization_samples.iter().map(|m| m * m).sum::<f64>()
            / magnetization_samples.len() as f64;
    ...
    pub fn print_3d_slice(&self, w_level: usize) {
        if w_level >= self.size {
            println!(
                "Error: w_level {} exceeds lattice size {}",
                w_level, self.size
            );
            return;
        }
        ...
        println!(
            "4D → 3D slice at w={} (showing {} z-slices):",
            w_level,
            self.size.min(4)
        );
        ...
        for z_slice in 0..num_z_slices {
            ...
            if z_level < self.size {
                ...
                for i in 0..self.size.min(8) {
                    // Limit display size
                    print!("    ");
                    for j in 0..self.size.min(8) {
                        print!(
                            "{:2}",
                            if self.spins[i][j][z_level][w_level] == 1 {
                                "↑"
                            } else {
                                "↓"
                            }
    ...
    pub fn print_2d_slice(&self, k_level: usize, l_level: usize) {
        ...
        for i in 0..self.size.min(12) {
            for j in 0..self.size.min(12) {
                print!(
                    "{:2}",
                    if self.spins[i][j][k_level][l_level] == 1 {
                        "↑"
                    } else {
                        "↓"
                    }
