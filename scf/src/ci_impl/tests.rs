...
mod tests {
    ...
    fn test_ci_initialization() {
        ...
        let ci = CI::new(mo_coeffs, orbital_energies, mo_basis, elems, hf_energy, 5, 1e-6);
    ...
    fn test_cis_calculation() {
        ...
        let mut ci = CI::new(mo_coeffs, orbital_energies, mo_basis, elems, hf_energy, 3, 1e-6);
        ...
        for &energy in &excitation_energies {
            assert!(energy > 0.0, "Excitation energy should be positive: {}", energy);
        }
        
        // Excited states should be ordered (lowest first)
        for i in 1..excitation_energies.len() {
            assert!(excitation_energies[i] >= excitation_energies[i-1],
                    "Excitation energies should be ordered");
        }
        
        // Check that excitation energies were stored
        assert_eq!(ci.get_excitation_energies().len(), excitation_energies.len());
    }
    ...
    fn test_cisd_calculation() {
        ...
        let mut ci = CI::new(mo_coeffs, orbital_energies, mo_basis, elems, hf_energy, 1, 1e-6);
    ...
    fn test_cis_no_virtual_orbitals() {
        ...
        let mut ci = CI::new(mo_coeffs, orbital_energies, mo_basis, elems, hf_energy, 5, 1e-6);
