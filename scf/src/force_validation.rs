use crate::optim_impl::{create_optimizer, CGOptimizer, GeometryOptimizer};
...
impl ForceValidator {
    ...
    pub fn validate_forces_comprehensive<S: SCF + Clone>(
        ...
        for (i, (ana, num)) in analytical_forces
            .iter()
            ...
            .enumerate()
        {
            ...
            info!(
                "   {:2}  | [{:8.4}, {:8.4}, {:8.4}] | [{:8.4}, {:8.4}, {:8.4}] | {:10.6}",
                i + 1,
                ana.x,
                ana.y,
                ana.z,
    ...
    pub fn test_optimization_convergence() -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!(
            "  Bond length: {:.6} bohr ({:.6} Å)",
            initial_bond_length,
            initial_bond_length * 0.529177
        );
        ...
        if max_error > 1e-2 {
            info!(
                "Warning: Large discrepancy in force calculation (max error: {:.6})",
                max_error
            );
        ...
        info!(
            "  Final bond length: {:.6} bohr ({:.6} Å)",
            final_bond_length,
            final_bond_length * 0.529177
        );
        ...
        let (final_analytical_forces, _, final_max_error) =
            Self::validate_forces_comprehensive(&mut scf, &optimized_coords, &elements, 1e-3);

        // Check convergence criteria
        let final_max_force = final_analytical_forces
            .iter()
            .map(|f| f.norm())
            .fold(0.0, f64::max);
    ...
    pub fn test_numerical_gradient_convergence() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for &delta in &step_sizes {
            let (_, numerical_forces, _) =
                Self::validate_forces_comprehensive(&mut scf, &coords, &elements, delta);
            ...
            info!(
                "{:8.0e} | {:9.6} | {:9.6} | {}",
                delta, max_error, rms_error, notes
            );
        }
