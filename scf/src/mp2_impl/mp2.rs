...
impl<B: Basis + Send + Sync> MP2<B> {
    ...
    pub fn calculate_mp2_energy_direct(&mut self) -> f64 {
        ...
        for i in 0..self.num_occ {
            for j in 0..i+1 { // j <= i to avoid double counting
        ...
        info!("Calculating MP2 energy for {} occupied and {} virtual orbitals...", 
        ...
        for i in 0..self.num_occ {
            for j in 0..=i {
                ...
                if count % 10 == 0 || count == self.num_occ * (self.num_occ + 1) / 2 {
                    info!("Processed {}/{} occupied orbital pairs, current E_MP2 = {:.12} Eh", 
                          count, self.num_occ * (self.num_occ + 1) / 2, total_energy);
                }
            }
        }
